...
from PyQt5.QtWidgets import QVBoxLayout, QHBoxLayout, QWidget, QLabel, QDialog, QSlider, QSpinBox, QDoubleSpinBox, \
    QPlainTextEdit, QCheckBox, QGridLayout, QGroupBox , QApplication, QPushButton, QLineEdit, QProgressBar, QMessageBox, QFileDialog
...
class ESCLogAnalysis(QDialog):
    ...
    def connect_can(self):
        ...
        node.periodic(0.04, self.publish_throttle_setpoint)
        node.periodic(1, self.publish_arm_state)
    ...
    def publish_throttle_setpoint(self):
        if self.arm_requested == True:
            ...
            if self.thr_out_list_pt < len(self.thr_out_list):
                setpoint = int(self.thr_out_list[self.thr_out_list_pt])
                self.thr_out_list_pt = self.thr_out_list_pt + 1

            self.thr_out_ls.append(setpoint)
            time = QDateTime.currentDateTime()
            time_display = time.toString('yyyy-MM-dd hh:mm:ss dddd')
            self.ts.append(time_display)

            self.thr_out_fig.add_data(setpoint) 
    ...
    def fetch_data(self,msg):
        ...
        if "uavcan.equipment.esc.Status" in str(payload):
            ...
            instance = msg.transfer.payload.esc_index
            if(self.motor_selection.text() == instance):
                ...
                self.telem_temp_ls.append(msg.transfer.payload.temperature)
                ...
                self._tele_temp.setText(self.telem_temp_ls[-1])
                self._tele_volt.setText(self.telem_volt_ls[-1])
                self._tele_curr.setText(self.telem_curr_ls[-1])
                self._tele_rpm.setText(self.telem_rpm_ls[-1])
                self._tele_status.setText(self.telem_error_ls[-1])
    ...
    def show_time(self):
        ...
        self.scan_progress.setValue((time_elapsed/int(self.run_time.value()))* 100)
    ...
    def file_save(self):
        ...
        if len(self.ts)> 0 : data_file['Time Stamp'] = self.ts
        if len(self.thr_out_ls)> 0 :data_file['Throttle Out'] = self.thr_out_ls
        if len(self.telem_rpm_ls)> 0 :data_file['RPM'] = self.telem_rpm_ls
        if len(self.telem_volt_ls)> 0 :data_file['Voltage'] = self.telem_volt_ls
        if len(self.telem_curr_ls)> 0 :data_file['Current'] = self.telem_curr_ls
        if len(self.telem_error_ls)> 0 :data_file['Error'] = self.telem_error_ls
        if len(self.telem_temp_ls)> 0 :data_file['Temperature'] = self.telem_temp_ls
    ...
    def _spin_node(self):
        ...
        try:
            ...
            msg = 'Node spin error [%d]: %r' % (self._successive_node_errors, ex)
